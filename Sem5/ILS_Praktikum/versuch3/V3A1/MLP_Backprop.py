...
def forwardPropagateActivity(x,W1,W2,flagBiasUnit=1): 
    ...
    """    
    a_1 = None                                  # REPLACE DUMMY CODE: compute dendritic potentials of hidden layer a_1
    z_1 = np.zeros(len(W1))                     # REPLACE DUMMY CODE: compute activity z_1 of hidden layer 1 
    if flagBiasUnit>0: z_1=np.append(z_1,[1.0]) # add bias unit (with constant activity 1) to hidden layer ?
    a_2 = None                                  # REPLACE DUMMY CODE: compute dendritic potentials of output layer a_2 
    z_2 = np.zeros(len(W2))+1e-5                # REPLACE DUMMY CODE: compute softmax activations for output layer 
...
def backPropagateErrors(z_1,z_2,t,W1,W2,flagBiasUnit=1): # backpropagate error signals delta_L
    ...
    delta_2=np.zeros(len(W2))                 # REPLACE DUMMY CODE: Initializing error signals in output layer 2  
    alpha_1=None                              # REPLACE DUMMY CODE: compute error potentials in hidden layer 1 by backpropagating errors delta_2
    h_prime=None                              # REPLACE DUMMY CODE: factor (1-z_1.*z_1) is h'(a) for tanh sigmoid function 
    delta_1=np.zeros(len(W1))                 # REPLACE DUMMY CODE: compute error signals in hidden layer 1
...
def doLearningStep(W1,W2,xn,tn,eta,lmbda_by_N=0,flagBiasUnit=1): # do one backpropagation learning step...
    ...
    nablaED_1 = None                             # REPLACE DUMMY CODE: gradient of data error function for first layer
    nablaED_2 = None                             # REPLACE DUMMY CODE: gradient of data error function for second layer
    W1=W1                                        # REPLACE DUMMY CODE: update weights for first layer with "weight decay" regularization
    W2=W2                                        # REPLACE DUMMY CODE: update weights for second layer with "weight decay" regularization
