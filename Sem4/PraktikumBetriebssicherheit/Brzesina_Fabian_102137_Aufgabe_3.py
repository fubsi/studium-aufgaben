...
class STATE:
    ...
    def utility(self):
        self.v = max([q.q for q in self.qstates])
        return self.v
...
class MDP:
    ...
    def utility(self):
        print('Utility before Value Iteration:')
        for s in self.states:
...
class GraphPrint:
    ...
    def create(self):
        ...
        for qstate in self.MDP.qstates:
            if qstate.action == 'betreiben':
                self.graph.node(qstate.name, shape='circle', color='blue')
                self.graph.edge(qstate.state.name, qstate.name, label=qstate.action, color='blue')
            else:
                self.graph.node(qstate.name, shape='circle', color='orange')
                self.graph.edge(qstate.state.name, qstate.name, label=qstate.action, color='orange')
        for transition in self.MDP.transitions:
            if transition.source.action == 'betreiben':
                self.graph.edge(transition.source.name, transition.destination.name, color="blue", label=f"{transition.name} - [{str(transition.prop)}]({str(transition.reward)})")
            else:
                self.graph.edge(transition.source.name, transition.destination.name, color="orange", label=f"{transition.name} - [{str(transition.prop)}]({str(transition.reward)})")
...
if __name__ == '__main__':
    ...
    #Zustände
    S_PARKEN_OI = STATE('Parken o.I.', 1)
    S_PARKEN_MI = STATE('Parken m.I.', 2)
    S_FAHREN_OI = STATE('Fahren o.I.', 3)
    S_FAHREN_MI = STATE('Fahren m.I.', 4)
